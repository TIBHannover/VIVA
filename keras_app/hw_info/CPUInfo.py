...
# === MODIFICATION of file starts here
def get_cpu_info():
    data_cpu = {}
    try:
        data_cpu['cpu_name'] = cpu.info[0]['model name']
        data_cpu['cpu_cores'] = psutil.cpu_count(False)
        data_cpu['cpu_count'] = psutil.cpu_count(True)
        data_cpu['cpu_pct'] = int(psutil.cpu_percent(interval=.7))
        data_cpu['mem_total'] = psutil.virtual_memory().total
        data_cpu['mem_use'] = psutil.virtual_memory().used
        data_cpu['mem_pct'] = int(data_cpu['mem_use'] * 100 / data_cpu['mem_total'])
        data_cpu['mem_swap_total'] = psutil.swap_memory().total
